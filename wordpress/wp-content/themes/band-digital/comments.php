...
<div id="comments" class="comments my-4">
	...
	if ( have_comments() ) :
		...
		<ol class="comment-list p-0">
			...
			wp_list_comments(
				array(
					'walker'            => new Bootstrap_Walker_Comment(), //шаблон коментов
                    'max_depth'         => '2', //вложенность 
                    'style'             => 'ol', //во что оборачиваем коменты
                    'type'              => 'all',
    ...
    $defaults = [
        'fields'               => [
            ...
            'email'  => '<div class="col-lg-6">
                <input id="email" name="email" class="form-control"type="email" value="' . esc_attr(  $commenter
                ['comment_author_email'] ) . '" size="30" aria-describedby="email-notes" placeholder = "E-mail" />
            </div></div>',
        ...
        'must_log_in'          => '<p class="must-log-in">' .
             sprintf( __( 'Вам нужно войти <a href="%s">войти</a> что бы оставить коментарий.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink($post->ID ) ) ) ) . '
         </p>',
        ...
        'name_submit'          => 'submit',
        'title_reply'          => __( 'Оставьте коментарий' ),
